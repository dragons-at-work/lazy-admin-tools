...

{
	...
	echo "listen on 0.0.0.0 port 587 tls-require auth <submission_auth>$PKI_REFS"
	echo "listen on ::    port 587 tls-require auth <submission_auth>$PKI_REFS"
} > "$OUT/smtpd-mailtls.conf"
chmod 644 "$OUT/smtpd-mailtls.conf"

echo "[OK] generated smtpd-mailtls.conf (${#PKI_NAMES[@]} domain(s))"
